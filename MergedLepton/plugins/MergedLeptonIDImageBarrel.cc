...
class MergedLeptonIDImageBarrel : public edm::one::EDAnalyzer<edm::one::SharedResources> {
public:
  explicit MergedLeptonIDImageBarrel(const edm::ParameterSet&);
  virtual ~MergedLeptonIDImageBarrel() {}
...
private:
  ...
  TTree* EBtree_ = nullptr;
  std::vector<float> EBenergy;
  std::vector<float> EBtime;
  std::vector<int> EBzside;
  std::vector<int> EBix;
  std::vector<int> EBiy;
  std::vector<float> EBx;
  std::vector<float> EBy;
  std::vector<float> EBz;

  TTree* ImageTree_ = nullptr;
  std::vector<std::vector<float>> EBImage_branch; 
...
MergedLeptonIDImageBarrel::MergedLeptonIDImageBarrel(const edm::ParameterSet& iConfig) :
...
bool MergedLeptonIDImageBarrel::extrapolate(const reco::GsfElectron& aEle, const reco::TrackBase& addTrk,
                                     const math::XYZPoint& beamSpotPos, const edm::EventSetup& iSetup,
                                     EleRelPointPair& scAtVtx, EleRelPointPair& seedAtCalo) {
...
void MergedLeptonIDImageBarrel::beginJob() {
  TH1::SetDefaultSumw2();
  ...
  EBtree_ = fs->make<TTree>("EBTree","EBTree");
  EBtree_->Branch("EBenergy",&EBenergy,32000,0);
  EBtree_->Branch("EBtime",&EBtime,32000,0);
  EBtree_->Branch("EBix",&EBix,32000,0);
  EBtree_->Branch("EBiy",&EBiy,32000,0);
  EBtree_->Branch("EBzside",&EBzside,32000,0);
  EBtree_->Branch("EBx",&EBx,32000,0);
  EBtree_->Branch("EBy",&EBy,32000,0);
  EBtree_->Branch("EBz",&EBz,32000,0);

  ImageTree_ = fs->make<TTree>("ImageTree","ImageTree");
  ImageTree_->Branch("EBImage",&EBImage_branch,32000,0);
...
void MergedLeptonIDImageBarrel::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (auto& crystal : *EBrecHitHandle){
    EBenergy.push_back(crystal.energy());
    ...
    EBz.push_back((float)crystalGeo->getPosition().z());
  ...
  for (const auto& electron : *emObjectHandle){
   ...
   // float phi_1st = reco::reduceRange( seedPosition.phi() - electron.deltaPhiSeedClusterTrackAtCalo());
    ...
    const EBDetId* matchedCrystal = nullptr;
...
DEFINE_FWK_MODULE(MergedLeptonIDImageBarrel);
