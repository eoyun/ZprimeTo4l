...
class GenParticle : public edm::one::EDAnalyzer<edm::one::SharedResources> {
public:
...
bool GenParticle::extrapolate(const reco::GsfElectron& aEle, const reco::TrackBase& addTrk,
                                     const math::XYZPoint& beamSpotPos, const edm::EventSetup& iSetup,
                                     EleRelPointPair& scAtVtx, EleRelPointPair& seedAtCalo) {
...
void GenParticle::beginJob() {
  ...
  GenTree_->Branch("genEta",&genEta,32000,0);
  GenTree_->Branch("genPhi",&genPhi,32000,0);
  GenTree_->Branch("genpT",&genpT,32000,0);
  GenTree_->Branch("genMass",&genMass,32000,0);

}

void GenParticle::endJob() {
  purwgtFile_->Close();
}
...
void GenParticle::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (unsigned int idx =0; idx<genptcHandle->size();++idx){
	  ...
	  if (genPtc->fromHardProcessFinalState()){
	  	genEta.push_back(genPtc->eta()); 
	  	genPhi.push_back(genPtc->phi()); 
	  	genpT.push_back(genPtc->pt()); 
		if (std::abs(genPtc->pdgId()) == 11) genMass.push_back(elmass_);
		if (std::abs(genPtc->pdgId()) == 13) genMass.push_back(mumass_);
		std::cout<<genPtc->eta()<<" | "<<genPtc->phi()<<" | "<<genPtc->pt()<<std::endl;
	  }
  }
  GenTree_->Fill();
