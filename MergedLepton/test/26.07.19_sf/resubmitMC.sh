...
do
	...
	#cp runMuonJpsi_run3_cfg.py ./crab/${name_list[$i]}
	cd ./crab/${name_list[$i]}
	#crab submit submit_crab.py
	#crab status -d crab_projects/crab_muonJpsi_${name_list[$i]} 
	crab resubmit -d crab_projects/crab_muonJpsi_${name_list[$i]} 
	cd ../..
done
