...
class tnpFitterJpsi {
public:
  tnpFitterJpsi( TH1 *hPass, TH1 *hFail, std::string histname  );
  ~tnpFitterJpsi(void) {if( _work != 0 ) delete _work; }
...
tnpFitterJpsi::tnpFitterJpsi(TH1 *hPass, TH1 *hFail, std::string histname  ) : _useMinos(false),_fixSigmaFtoSigmaP(false) {
...
void tnpFitterJpsi::setWorkspace(std::vector<std::string> workspace) {
...
void tnpFitterJpsi::fits(string title) {
...
void tnpFitterJpsi::textParForCanvas(RooFitResult *resP, RooFitResult *resF,TPad *p) {
  double eff = -1;
  double e_eff = 0;
  ...
  TPaveText *text1 = new TPaveText(0,0.8,1,1); // 0.9
