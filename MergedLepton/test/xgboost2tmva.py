...
npz_params = np.load('npz/'+os.path.splitext(os.path.basename(argv[1]))[0]+'.npz',allow_pickle=True)
...
bst.load_model(argv[1])
model = bst.get_dump()
print(model)
convert_model(model,input_variables=varlist,output_xml='model/'+os.path.splitext(os.path.basename(argv[1]))[0]+'.xml')

np_meanstd = npz_params['meanstd'][()]
np.savetxt('npz/'+os.path.splitext(os.path.basename(argv[1]))[0]+'.csv',np_meanstd,delimiter=' ')
