...
void estimateRMFFfromTTree(TString era) {
  setTDRStyle();
  ...
  public:
    ...
    : era_(aera) {
      dataFile_ = new TFile("MuAnalyzer_"+aera+"_data.root","READ");
      WZFile_ = new TFile("MuAnalyzer_"+aera+"_WZFXFX.root","READ");
      ZZFile_ = new TFile("MuAnalyzer_"+aera+"_ZZ.root","READ");
    }
    ...
    TH1D* fillHist(TFile* afile, const TString& treeName, std::vector<double>& binning, const double xsec=0.) {
      ...
      const TString anlyzrName = (xsec==0.) ? "resolvedMuCRanalyzerData" : "resolvedMuCRanalyzer"+aera;
  ...
  std::vector<double> xbins = {0.,20.,25.,30.,40.,50.,70.,100.,250.};
  ...
  result->SetMinimum(0.);
  result->GetXaxis()->SetTitle("p_{T}(#mu) [GeV]");
  result->Draw("E1");

  TF1* func = new TF1("RMFFfunc","[0]+[1]*TMath::Exp(-[2]*x)",20.,500.);
  ...
  SaveAs(canvas_2,"rmff_run2.pdf");

  // return;

  // save file
  TH1D* outhist = (TH1D*)result->Clone("RMFF_pt");
  const TString outname = "RMFF_"+era+".root";
