...
void drawlimit_PRL() {
  ...
  auto retrieve = [] (const TString& atype, const TString& hmass, const TString& amass, const TString& isZA, const TString& quant) -> double {
  ...
  auto graphNominal = [&retrieve,&massvec] (const TString& atype, const TString& amass, const TString& isZA, const TString& obs) -> std::unique_ptr<TGraph> {
    std::vector<double> limits, massdouble;

    for (unsigned idx=0; idx<massvec.size(); idx++) {
      double val = retrieve(atype,std::to_string(massvec.at(idx)),amass,isZA,obs);
  ...
  auto graph1sig = [&retrieve,&massvec] (const TString& amass, const TString& isZA) -> std::unique_ptr<TGraphAsymmErrors> {
    std::vector<double> x0, y0, errydn1sig, erryup1sig, errx;

    for (unsigned idx=0; idx<massvec.size(); idx++) {
      double nominal = retrieve("",std::to_string(massvec.at(idx)),amass,isZA,"0.500");
      double quant0p16 = retrieve("",std::to_string(massvec.at(idx)),amass,isZA,"0.160");
      double quant0p84 = retrieve("",std::to_string(massvec.at(idx)),amass,isZA,"0.840");
  ...
  auto graph2sig = [&retrieve,&massvec] (const TString& amass, const TString& isZA) -> std::unique_ptr<TGraphAsymmErrors> {
    std::vector<double> x0, y0, errydn2sig, erryup2sig, errx;

    for (unsigned idx=0; idx<massvec.size(); idx++) {
      double nominal = retrieve("",std::to_string(massvec.at(idx)),amass,isZA,"0.500");
      double quant0p025 = retrieve("",std::to_string(massvec.at(idx)),amass,isZA,"0.025");
      double quant0p975 = retrieve("",std::to_string(massvec.at(idx)),amass,isZA,"0.975");
  ...
  std::vector<TString> amassvec = {"0p8","10","100"};
  std::vector<TString> selectZA = {"A","Z"};
  ...
  for (int i = 0; i < cols; ++i) {
    for (int j = 0; j < rows; ++j) {
      ...
      auto gr1 = graph1sig(amassvec.at(i),selectZA.at(j));
      auto gr2 = graph2sig(amassvec.at(i),selectZA.at(j));
      auto grNominal = graphNominal("",amassvec.at(i),selectZA.at(j),"");
      auto grNominalExp = graphNominal("",amassvec.at(i),selectZA.at(j),"0.500");

      pad->SetLogy();

      TString Ychar = selectZA.at(j)=="A" ? "Y" : "Z";
      ...
      if (i==2 && j==0) {
        auto* legend = new TLegend(0.05,0.77,0.99,0.98);
        ...
        auto btext = std::make_unique<TPaveText>(0.15,0.63,0.99,0.75,"NDC");
      ...
      TString amassDecimal = amassvec.at(i);
      amassDecimal.ReplaceAll("p",".");

      double mar = (i==0) ? 0.1 : 0.;
      double mary = (j==1) ? 0.08 : 0.;

      double xoffset = (j==1) ? 0.45 : 0.;
      double yoffset = (j==1) ? 0.68 : 0.;

      if (i==0 && j==1)
        xoffset -= 0.035;

      auto atext = std::make_unique<TPaveText>(mar+0.02+xoffset,mary+0.09+yoffset,mar+0.6+xoffset,mary+0.19+yoffset,"NDC");
      ...
      ((TText*)atext->GetListOfLines()->Last())->SetTextAlign(11);
      ((TText*)atext->GetListOfLines()->Last())->SetTextSize(0.08);
      atext->Draw();
      atext.release();

      auto dtext = std::make_unique<TPaveText>(mar+0.02+xoffset,mary-0.02+yoffset,mar+0.6+xoffset,mary+0.11+yoffset,"NDC");
      dtext->SetBorderSize(0);
      dtext->SetFillColor(0);
      dtext->SetFillStyle(0);

      TString dstr = "X#rightarrow"+Ychar+"Y#rightarrow4l";
      dtext->AddText(dstr);
      ((TText*)dtext->GetListOfLines()->Last())->SetTextColor(kBlack);
      ((TText*)dtext->GetListOfLines()->Last())->SetTextAlign(11);
      ((TText*)dtext->GetListOfLines()->Last())->SetTextSize(0.08);
  ...
  SaveAs(canvas1,std::string((TString("limit")+"_PRL_"+era+".pdf").Data()));
