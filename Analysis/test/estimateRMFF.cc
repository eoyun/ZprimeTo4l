...
void estimateRMFF(TString era) {
  ...
  std::vector<double> xbinsMB = {0,10,20,35,50,75,100,150,200,300,500,700,1000};
  const int nbinsMB = xbinsMB.size()-1;
  std::vector<double> xcenMB = estimateCenter(xbinsMB);
  TH1D* MB_3P0F_rebin = (TH1D*)MB_3P0F->Rebin(nbinsMB, "MB_3P0F_rebin", &(xbinsMB[0]));
  TH1D* MB_2P1F_rebin = (TH1D*)MB_2P1F->Rebin(nbinsMB, "MB_2P1F_rebin", &(xbinsMB[0]));
  ...
  TH1D* ME_3P0F_rebin = (TH1D*)ME_3P0F->Rebin(nbinsMB, "ME_3P0F_rebin", &(xbinsMB[0]));
  TH1D* ME_2P1F_rebin = (TH1D*)ME_2P1F->Rebin(nbinsMB, "ME_2P1F_rebin", &(xbinsMB[0]));
  ...
  TFitResultPtr fitResultMBpt = MB_3P0F_rebin->Fit(fitMB,"RS");
  double ciMBpt[nbinsMB+1];
  fitResultMBpt->GetConfidenceIntervals(nbinsMB+1,1,0,&(xcenMB[0]),ciMBpt,0.95,false);

  std::vector<double> xbinwMB = estimateWidth(xbinsMB);
  double ybinMB[nbinsMB+1];

  for (unsigned idx = 0; idx < nbinsMB+1; idx++) {
    ybinMB[idx] = fitMB->Eval(xcenMB[idx]);
  }

  auto errMBpt = new TGraphErrors(nbinsMB+1,&(xcenMB[0]),&(ybinMB[0]),&(xbinwMB[0]),ciMBpt);
  errMBpt->SetFillColor(kRed);
  errMBpt->SetFillStyle(3004);

  TFitResultPtr fitResultMEpt = ME_3P0F_rebin->Fit(fitME,"RS");
  double ciMEpt[nbinsMB+1];
  fitResultMBpt->GetConfidenceIntervals(nbinsMB+1,1,0,&(xcenMB[0]),ciMEpt,0.95,false);

  double ybinME[nbinsMB+1];

  for (unsigned idx = 0; idx < nbinsMB+1; idx++) {
    ybinME[idx] = fitME->Eval(xcenMB[idx]);
  }

  auto errMEpt = new TGraphErrors(nbinsMB+1,&(xcenMB[0]),&(ybinME[0]),&(xbinwMB[0]),ciMEpt);
  errMEpt->SetFillColor(kBlue);
  errMEpt->SetFillStyle(3005);
  ...
  TPaveText* textMB = new TPaveText(0.15,0.6,0.5,0.75,"NDC");
  ...
  TPaveText* textME = new TPaveText(0.6,0.6,0.92,0.75,"NDC");
  ...
  TPaveText* textDRall = new TPaveText(0.45,0.6,0.9,0.75,"NDC");
  textDRall->SetBorderSize(0);
  textDRall->SetFillColor(0);
  TString textDRallStrBelow;
  textDRallStrBelow.Form("%.3g #times #DeltaR + %.3g (#DeltaR < 0.8)", fitDRall->GetParameter(2), fitDRall->GetParameter(0)*0.8+fitDRall->GetParameter(1)-fitDRall->GetParameter(2)*0.8);
  TString textDRallStrAbove;
  textDRallStrAbove.Form("%.3g #times #DeltaR + %.3g (#DeltaR > 0.8)", fitDRall->GetParameter(0), fitDRall->GetParameter(1));
  ...
  canvas->SaveAs("RMFF_all_dr_"+era+".pdf");
